    ...
    for dosya_yolu in pdf_dosyalari:
        with pdfplumber.open(dosya_yolu) as pdf:
            for i, sayfa in enumerate(pdf.pages):
                ...
                unique_key = (sinif_bilgisi, ders_adi)
                if unique_key in islenen_dersler_listesi:
                    print(f"ATLANDI: Sayfa {i+1} -> Sınıf: {sinif_bilgisi} | Ders: {ders_adi} (Zaten alındı)")
                    continue
                ...
                for satir in tablo:
                    ...
                    # --- DİNAMİK SÜTUN BULUCU (MÜKEMMEL ÇÖZÜM) ---
                    # Sıra No'nun nerede olduğunu (0. index mi, 1. index mi) buluyoruz.
                    sira_no_index = -1
                    ...
                    # Eğer iki indexte de sayı yoksa, bu bir başlıktır, atla.
                    if sira_no_index == -1:
                        continue
                    ...
                    try:
                        # --- TÜM NOTLARI VERİTABANI ŞEMASINA UYGUN ÇEKİYORUZ ---
                        ogrenci_verisi = {
                            ...
                            "muaf": satir[sira_no_index + 16], # Genelde "M" veya metin olur
                ...
                print(f"OKUNDU: Sayfa {i+1} -> Sınıf: {sinif_bilgisi} | Ders: {ders_adi} ({satir_sayaci} Öğrenci)")
...
if __name__ == "__main__":
    ...
    # Klasör yoksa hata vermemesi için test
    if not os.path.exists(klasor):
        ...
        if not df.empty:
            ...
            # Çıktıda tüm notları gösterelim
            print(df[['ogrenci_no', 'ad_soyad', 'y1', 'y2', 'y3', 'y4', 'y5', 'os','p1','p2','p3','u1', 'u2','u3','proje', 'ortalama']].head())
